...
RonaMove::RonaMove() {
	...
	privNh.param("pub_state_topic", pub_name_state,
			std::string("rona/move/state"));
	privNh.param("pub_process_topic", pub_name_process,
			std::string("rona/move/process"));
	privNh.param("pub_name_marker", pub_name_marker,
			std::string("rona/robot_marker"));
	privNh.param("sub_path_topic", sub_name_path,
			std::string("rona/move/path"));
	privNh.param("sub_ctrl_topic", sub_ctrl_topic,
			std::string("rona/move/ctrl"));
	privNh.param("map_frame", tf_map_frame, std::string("map"));
	privNh.param("robot_frame", tf_robot_frame, std::string("base_footprint"));
	privNh.param("robot_reverse_frame", tf_robot_reverse_frame,
			std::string("base_footprint_reverse"));
	...
	ROS_INFO("use_tf_stamp_offset: %s",
			_use_tf_stamp_offset ? "true" : "false");

	//init publisher
	_pub_cmd_vel = _nh.advertise < geometry_msgs::Twist
			> (pub_name_cmd_vel, 10); //todo heck if twist stamped is needed...
	...
	//inti subscriber
	_sub_path = _nh.subscribe(sub_name_path, 10, &RonaMove::subPath_callback,
			this);
	_sub_ctrl = _nh.subscribe(sub_ctrl_topic, 1000, &RonaMove::subCtrl_callback,
			this);
	_sub_pause = _nh.subscribe("rona/move/pause", 1,
			&RonaMove::subPause_callback, this);

	_srv_pause = _nh.advertiseService("rona/move/pause",
			&RonaMove::srvPause_callback, this);
	_srv_node_ctrl = _nh.advertiseService("/rona/move/node_ctrl",
			&RonaMove::srvNodeCtrl_callback, this);
	_srv_reverse_on = _nh.advertiseService("/rona/move/set_reverse_on",
			&RonaMove::srvReverseOn_callback, this);
	_srv_reverse_off = _nh.advertiseService("/rona/move/set_reverse_off",
			&RonaMove::srvReverseOff_callback, this);
	_srv_reverse_sw = _nh.advertiseService("/rona/move/set_reverse_sw",
			&RonaMove::srvReverseSw_callback, this);
	...
	if (kinematic == "differential") {
		...
		_pathAnalyser =
				std::make_unique < analyser::BasicAnalyser
						> (target_radius, target_radius_final, cos_pwr_n, cos_fac_n, ang_reached_range, lin_end_approach);
		_pathAnalyser->setDoEndRotate(do_endrotate);

		_controller = std::make_unique < controller::ParabolaTransfere
				> (vel_lin_max, vel_ang_max, lin_ctrl_scale, ang_ctrl_scale);
...
//    cfg.hold_pos              = hold_pos;
		...
		_controller = std::make_unique < controller::LinPwrTransfere
				> (vel_lin_max, vel_ang_max, lin_ctrl_scale, ang_ctrl_scale);
	} else {
	...
	_callBackConfig = boost::bind(&RonaMove::callbackDynamicReconfigure, this,
			_1, _2);
...
void RonaMove::start() {
	...
	do {
		try {
			...
			_tf_listnener.waitForTransform(_tf_map_frame, _tf_robot_frame, time,
					ros::Duration(1));
			rdy = true;
	...
	_loopTimer = _nh.createTimer(ros::Duration(_loop_duration),
			&RonaMove::timerLoop_callback, this);
	ros::spin();
}
...
void RonaMove::timerLoop_callback(const ros::TimerEvent& e) {
	...
	//pub Marker
	rona::map::Point2D pose = rona::Utility::getTransformPoint2D(_tf_listnener,
			_tf_map_frame, _tf_robot_frame);
...
void RonaMove::doPathControl() {
	...
	try {
		...
		if (_use_tf_stamp_offset) {
			if (_tf_stamp_offset < 0)
				time = ros::Time::now()
						- ros::Duration(std::abs(_tf_stamp_offset));
			else
				time = ros::Time::now()
						+ ros::Duration(std::abs(_tf_stamp_offset));
	...
	Quaternion<double> tmp_q(tf.getRotation().w(), tf.getRotation().x(),
			tf.getRotation().y(), tf.getRotation().z());
	pose.orientation =
			analyser::PathAnalyser_base::quaternion_to_orientationVec(tmp_q);
...
//  std::cout << "lin_y : " << diff_scale.linear_y << std::endl;
	...
	//controll diffscale
	controller::velocity vel = _controller->control(diff_scale.linear_x,
			diff_scale.linear_y, diff_scale.angular);
...
void RonaMove::subPath_callback(const nav_msgs::Path& msg_) {
	...
	for (unsigned int i = 0; i < msg.poses.size(); i++) {
		...
		tmp_pose.position = Vector3d(msg.poses[i].pose.position.x,
				msg.poses[i].pose.position.y, msg.poses[i].pose.position.z);
		...
		tmp_pose.orientation =
				analyser::PathAnalyser_base::quaternion_to_orientationVec(
						tmp_q);
	...
	do {
		try {
			ros::Time time = ros::Time(0);
			_tf_listnener.lookupTransform(_tf_map_frame, _tf_robot_frame, time,
					tf);
	...
	for (int i = (int) path_comp.size() - 1; i >= 0; --i) {
		...
		if ((cx - px) * (cx - px) + (cy - py) * (cy - py)
				< (detectionRaidus * detectionRaidus)) {  //point is in radius
	...
	if (!path_trunc.size()) { //if path_tranc contains no points than use path_comp;
...
bool RonaMove::srvPause_callback(std_srvs::SetBool::Request& req,
...
bool RonaMove::srvNodeCtrl_callback(rona_msgs::NodeCtrlSRVRequest& req,
		rona_msgs::NodeCtrlSRVResponse& res) {
	res.accepted = this->processNodeCtrl(req.ctrl);
	return true;
}

bool RonaMove::srvReverseOn_callback(std_srvs::EmptyRequest& req,
...
bool RonaMove::srvReverseOff_callback(std_srvs::EmptyRequest& req,
		std_srvs::EmptyResponse& res) {
...
bool RonaMove::srvReverseSw_callback(std_srvs::EmptyRequest& req,
		std_srvs::EmptyResponse& res) {
	ROS_INFO("RonaMove -> sw reverseMode caled -> curr state: %s",
			_reverseMode ? "true" : "false");
	_reverseMode = !_reverseMode;
	ROS_INFO("RonaMove -> sw reverseMode caled -> after state: %s",
			_reverseMode ? "true" : "false");
	return true;
}
...
bool RonaMove::processNodeCtrl(const rona_msgs::NodeCtrl& msg) {
	if (msg.cmd == msg.PAUSE) {
		...
		return false;
	} else if (msg.cmd == msg.CONTINUE) {   //continue only if is in pause state
		if (_state == State::PAUSE) {
			_state = State::MOVE;
			return true;
		}
...
void RonaMove::callbackDynamicReconfigure(rona_move::MoveConfig& config,
		uint32_t level) {
	_pathAnalyser->setConfig(config.target_radius, config.target_radius_final,
			config.cos_pwr_n, config.cos_fac_n);
	_controller->setConfig(config.vel_lin_max, config.vel_ang_max,
			config.lin_ctrl_scale, config.ang_ctrl_scale);
}
